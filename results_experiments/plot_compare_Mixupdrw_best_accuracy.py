...
def clean_label(filename):
    """
    Extracts exactly the epoch number (130, 140, etc.) from the filename.
    Looks for the number immediately following 'drw'.
    """
    match = re.search(r'drw(\d+)', filename.lower())
    if match:
        return match.group(1)
    return filename

def get_epoch_for_sorting(filename):
    """
    Helper function to turn the extracted text into a real integer for perfect ascending sorting.
    """
    match = re.search(r'drw(\d+)', filename.lower())
    if match:
...
def plot_multi_comparison(source_paths, output_dir):
    ...
    # ---------------------------------------------------------
    # ✨ THE FIX: Sort mathematically by the extracted epoch in ascending order
    # ---------------------------------------------------------
    results = sorted(results, key=lambda x: get_epoch_for_sorting(x['name']))
    ...
    # ✨ Changed the X-axis Title to "epoch"
    ax.set_xlabel('epoch', fontweight='bold', fontsize=12) 
    
    ax.set_title('Imbalanced cifar10 Mixup_DRW epochs Lava 0.7', fontsize=16, fontweight='bold')
    ax.set_xticks(x)
...
if __name__ == "__main__":
    ...
    folders_to_compare = [
        "../experiments/Mixup_drw/drw130",
        "../experiments/Mixup_drw/drw150"
    ]
    
    results_output = "Mixup_drw/comparisons" 
