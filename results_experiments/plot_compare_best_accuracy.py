...
def clean_label(filename):
    """
    Removes file extensions and common timestamp patterns to keep the chart clean.
    """
    name = filename.replace(".log", "")
    name = re.sub(r'_\d{8}_\d{6}', '', name)
    name = name.replace("cifar10_", "").replace("cifar100_", "")
    return name
...
def plot_multi_comparison(source_paths, output_dir):
    ...
    # ---------------------------------------------------------
    # ✨ THE FIX: Force 'none' to the front, sort the rest normally
    # ---------------------------------------------------------
    results = sorted(results, key=lambda x: (0 if 'none' in x['name'].lower() else 1, x['name']))
    ...
    ax.set_title('Imbalanced cifar10 RandOversamp_2 noisy and Lava selection ratios', fontsize=16, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontweight='bold', rotation=30, ha='right')
    ax.set_ylim(0, max(max(train_scores), max(test_scores)) + 15)
    ax.legend(loc='lower right')

    plt.tight_layout()
    save_path = os.path.join(output_dir, 'Imbalanced cifar10 RandOversamp_2 noisy and Lava selection ratios.png')
...
if __name__ == "__main__":
    ...
    folders_to_compare = [
        "../experiments/cifar10_exp0.01_noisy_randOversamp_2",
        "../experiments/cifar10_exp0.01_noisy_randOversamp_2/lava"
    ]
    
    results_output = "cifar10_exp0.01_noisy_randOversamp_2/comparisons" 
