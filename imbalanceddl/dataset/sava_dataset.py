# imbalanceddl/dataset/sava_dataset.py (modified __init__ and added attribute)
...
class SavaDataset(Dataset):
    def __init__(self, config, base_dataset, ratio, method, device='cuda'):
        ...
        # ----- Initialize scores attribute (will be set by selection) -----
        self.scores = None   # raw SAVA scores in original order
        ...
        if method_str == 'sava':
            ...
            # Call selection with return_scores=True
            indices, raw_scores = get_sava_selection_indices(
                train_dataset=no_aug_train_dataset,
                val_dataset=val_ds,
                keep_ratio=self.ratio,          # still used for subsetting
                device=self.device,
                file_key=file_key,
            ...
            # Store scores for reweighting
            self.scores = raw_scores   # shape (N,) in original dataset order
            ...
            if self.debug:
                ...
                self.logger.debug(f"First 10 scores (original order): {self.scores[:10] if self.scores is not None else 'None'}")
            ...
            # Random selection does not provide scores
            self.scores = None
            if hasattr(train_ds, 'targets'):
                selected_targets = np.array(train_ds.targets)[indices]
                unique, counts = np.unique(selected_targets, return_counts=True)
