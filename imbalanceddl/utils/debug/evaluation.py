...
@torch.no_grad()
def extract_data(model, gate, loader, T, la_tau, log_prior, log_spc, k, device):
    all_logits = [[], [], []]
    all_embeddings = []
    all_labels = []
    all_weights = []
...
def run_temperature_comparison(T, l_ce_test, l_la_test, l_bs_test, w_test, k, log_prior, log_spc, labels_test, cfg, train_dataset, m_unif, m_method):
    ...
    p_la_T1 = F.softmax(l_la_test + log_prior.cpu(), dim=1)
...
def run_saves_the_day_checks(p_ce_test, p_la_test, p_bs_test, w_test, labels_test, label_groups_test, k):
    ...
    for i in la_saves_day_indices[:15]:
        ...
        mix_pred = np.argmax(p_la_test[i]) # Just to show LA's correct pred
