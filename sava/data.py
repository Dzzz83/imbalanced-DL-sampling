...
def get_indices(singleloader):
    # Try to get indices from batch_sampler.sampler if it exists
    if hasattr(singleloader, 'batch_sampler') and hasattr(singleloader.batch_sampler, 'sampler'):
        sampler = singleloader.batch_sampler.sampler
        if hasattr(sampler, 'indices'):
            return sampler.indices
    # Fallback: return a list of indices from the dataset length
