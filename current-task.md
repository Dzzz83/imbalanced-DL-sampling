...
Modify `ExpertEnsemble.forward()` to return per-expert `hidden_list` (3 × 64-dim embeddings) — currently the `hidden_list` is collected at line ~112 but discarded after averaging. Then run the same correlation diagnostics as `diagnose_feature_collinearity.py` on the 192-dim embedding space:
...
> **Note:** This file was updated after Exp 17. DaWin confidence routing is conclusively ruled out. The next step is to run the 192-dim embedding correlation check (Exp 18, Phase A) to determine whether penultimate feature routing is viable, or whether the project must pivot to expert diversification.
