...
def main():
    ...
    # checkpoints use (probability vs penultimate). This affects how we cache
    # the model's outputs and build gate embeddings.
    ...
    if first_mode == 'penultimate':
        # Penultimate mode: cache the three 64-dim hidden states once (they
        # don't depend on calibration params, so they're the same for every
        # checkpoint in this directory). Also cache logits for mixture math.
        ...
        print(f"[INFO] Cached {labels.shape[0]} samples with "
              ...
              f"and raw logits for mixture computation.")
    else:
        # Probability mode: cache raw logits, then rebuild embeddings per
        # checkpoint with each checkpoint's specific calibration params.
    ...
    for g_path in gate_files:
        ...
        with torch.no_grad():
            if recipe['gate_input_mode'] == 'penultimate':
                ...
                # using the checkpoint's calibration params.
